...
`generate.py`. Note that for each subdirectory in script-test there is a matching directory
under `bluetooth/`. The generator will expand `CALLS` functions into the
...
by implementing the [Web Bluetooth Testing], even if only on their internal
infrastructure.
